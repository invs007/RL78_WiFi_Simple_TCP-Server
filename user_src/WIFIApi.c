...
uint8_t Response_handle();           // Handle Basic Repsonse (OK or ERROR) 
void Data_handle(uint8_t);           // Handle Data Response
void IP_extract(uint8_t *,uint8_t);
extern void SPI_CS_Start(uint8_t);  // Enable Chipselect for Device 
extern void SPI_CS_End(uint8_t);    // Disable Chipselect for Device
void Disp_Client_Data();
uint8_t Parse_Data(char);           // Parse Client Messages
...
extern uint8_t WIFI_ReceiveByte(char*);

/***********************************************************************************************************************
* Function Name: wait_for_sw()
* Description  : wait for Switch 2 press
* Arguments    : None
* Return Value : none
***********************************************************************************************************************/
...
uint8_t Echo_Disable()
{
  ...
  while(outbuf[i] != '\0')
  {
    SPI_Send(SPI_WIFI,(uint8_t *) &outbuf[i], 1);
    i++;
  }
 if(!(Response_handle()))
      {   
...
uint8_t Received_Char_Handle(char buf)
{
